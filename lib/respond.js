...
const respond = (request, response) => {
  ...
  //the boolean true is helping us to manipulate the query...it gets converted into an object which is easier to manipulate
  ...
  //now we make a decoded pathname which makes more sense
  ...
  let stats;
  try {
    //this returns a bunch of statistics which we can use
  ...
  if (stats.isDirectory()) {
    ...
    let pathElements = pathname.split("/").reverse(); //we reverse it so that the name we want appears in the beginning of the array
    pathElements = pathElements.filter((element) => element !== ""); //getting rid of empty element
    ...
    response.statusCode = 200; //ok status response.....the response has succeeded...like 404 is for failure
    response.write(data);
    return response.end();
  }
